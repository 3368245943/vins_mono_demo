...
void ViewController::shutdown() {
    ...
    if (saveData.joinable()) saveData.join();
    if (loop_thread.joinable()) loop_thread.join();
    if (globalLoopThread.joinable()) globalLoopThread.join();
}
...
void ViewController::processImage(cv::Mat &image, double timeStamp, bool isScreenRotated,
                                  bool renderPointCloud, cv::Mat *featurePreview) {

    TS(ViewController_processImage);
    last_frame_is_map = false;
    ...
    if (isCapturing) {
        ...
        if (image.size() != frameSize) {
            const cv::Size swappedSize(frameSize.height, frameSize.width);
            if (image.size() == swappedSize) {
                cv::rotate(image, image, cv::ROTATE_90_CLOCKWISE);
            } else {
                LOGE("Unexpected camera frame size %dx%d; resizing to %dx%d",
                     image.cols, image.rows, frameSize.width, frameSize.height);
        ...
        if (featurePreview != nullptr) {
            image.copyTo(*featurePreview);
        }
        TS(visualize);
        TS(imageCaching);
        if (imageCacheEnabled) {
        ...
        // Non-preview frames still feed VINS, but do not spend time rebuilding UI output.
        const bool showMap = renderPointCloud && vins.solver_flag == VINS::NON_LINEAR;
        if (!showMap) {
            if (isScreenRotated)
                cv::rotate(image, image, cv::ROTATE_180);
        } else {
            ...
            vector<int> trajectory_segments;
            std::array<Vector3f, WINDOW_SIZE> window_Ps;
            std::array<Matrix3f, WINDOW_SIZE> window_Rs;
            bool map_ready;
            {
                std::lock_guard<std::mutex> render_lock(m_render_feedback);
                point_cloud = render_point_cloud;
                trajectory = render_path;
                trajectory_segments = render_path_segments;
                window_Ps = render_window_Ps;
                window_Rs = render_window_Rs;
                map_ready = render_map_ready;
            }

            // Keep position, trajectory and point cloud on the same optimized
            // VINS snapshot. The front-end pose is an IMU propagation when PnP
            // is disabled; rendering it here makes a stationary phone continue
            // moving until the next optimization snaps the map back.

            if (!map_ready || trajectory.empty()) {
                if (!map_preview_cache.empty()) {
                    map_preview_cache.copyTo(image);
                    last_frame_is_map = true;
                }
                TE(visualize);
            ...
            TS(vins_draw);
            vins.drawresult.pose = trajectory;
            vins.drawresult.segment_indexs = trajectory_segments;
            vins.drawresult.origin_w[0] = window_Ps.back()[0];
            vins.drawresult.origin_w[1] = window_Ps.back()[1];
//                vins.drawresult.Fx = 85;
//                vins.drawresult.Fy = 85;
            vins.drawresult.radius = virtualCamDistance;
            vins.drawresult.theta = 75;
            vins.drawresult.phy = 75;
            vins.drawresult.Reprojection(vins.image_show, point_cloud,
                                         window_Rs.data(), window_Ps.data(), box_in_trajectory);
...
//            TE(big_Window_rotate);
            ...
            // Camera2 is converted to CV_8UC3/BGR in ImageUtils. Keep the map
            // in the same format so JNI can return it to the Java Mat.
            map_preview_cache = image.clone();
            last_frame_is_map = true;
        }
...
void ViewController::drawPointCloudOverlay(cv::Mat &image) {
    vector<Vector3f> point_cloud;
    Vector3f camera_P;
    Matrix3f camera_R;
    {
        std::lock_guard<std::mutex> render_lock(m_render_feedback);
        if (!render_map_ready || render_point_cloud.empty())
            return;
        point_cloud = render_point_cloud;
        camera_P = render_window_Ps.back();
        camera_R = render_window_Rs.back();
    }

    const Matrix3f ric = Utility::ypr2R(Vector3d(RIC_y, RIC_p, RIC_r)).cast<float>();
    const Matrix3f world_to_camera = (camera_R * ric).transpose();
    const Vector3f camera_offset(TIC_X, TIC_Y, TIC_Z);
    const size_t stride = std::max<size_t>(1, point_cloud.size() / 600);

    for (size_t i = 0; i < point_cloud.size(); i += stride) {
        const Vector3f point_in_camera = world_to_camera *
                (point_cloud[i] - camera_P - camera_R * camera_offset);
        if (point_in_camera.z() <= 0.10f)
            continue;

        const int x = static_cast<int>(FOCUS_LENGTH_X * point_in_camera.x() /
                                       point_in_camera.z() + PX);
        const int y = static_cast<int>(FOCUS_LENGTH_Y * point_in_camera.y() /
                                       point_in_camera.z() + PY);
        if (x >= 2 && x < image.cols - 2 && y >= 2 && y < image.rows - 2)
            cv::circle(image, cv::Point(x, y), 2, cv::Scalar(0, 230, 120, 255), -1);
    }
}
...
//                return (measurements = getMeasurements()).size() != 0;
    ...
    for (auto &measurement : measurements) {
        ...
        if (vins.solver_flag == VINS::INITIAL) {
            std::lock_guard<std::mutex> render_lock(m_render_feedback);
            render_point_cloud.clear();
            render_path.clear();
            render_path_segments.clear();
            render_map_ready = false;
            map_preview_cache.release();
        }

        // Android does not use the original iOS showInputView callback. Promote
        // the completed VINS state here so rendering and pose output begin.
        if (vins.solver_flag == VINS::NON_LINEAR && !start_show) {
            start_show = true;
            LOGI("VINS tracking visualization enabled");
        }
        ...
        if (vins.solver_flag == vins.NON_LINEAR) {
            LOGI("VINS_STATE P %.6f %.6f %.6f V %.6f %.6f %.6f "
                 "BA %.6f %.6f %.6f BG %.6f %.6f %.6f G %.6f %.6f %.6f "
                 "VCOST %.9f VF %d COST %.9f",
                 vins.Ps[WINDOW_SIZE].x(), vins.Ps[WINDOW_SIZE].y(),
                 vins.Ps[WINDOW_SIZE].z(), vins.Vs[WINDOW_SIZE].x(),
                 vins.Vs[WINDOW_SIZE].y(), vins.Vs[WINDOW_SIZE].z(),
                 vins.Bas[WINDOW_SIZE].x(), vins.Bas[WINDOW_SIZE].y(),
                 vins.Bas[WINDOW_SIZE].z(), vins.Bgs[WINDOW_SIZE].x(),
                 vins.Bgs[WINDOW_SIZE].y(), vins.Bgs[WINDOW_SIZE].z(),
                 vins.g.x(), vins.g.y(), vins.g.z(),
                 vins.visual_factor_num > 0
                         ? vins.visual_cost / vins.visual_factor_num : -1.0,
                 vins.visual_factor_num, vins.final_cost);
            m_depth_feedback.lock();
...
//                        Vector3d pts_i = it_per_id.estimated_depth * it_per_id.feature_per_frame[0].point;
            ...
            {
                std::lock_guard<std::mutex> render_lock(m_render_feedback);
                const size_t max_render_points = 180;
                const size_t stride = std::max<size_t>(1,
                        vins.correct_point_cloud.size() / max_render_points);
                render_point_cloud.clear();
                render_point_cloud.reserve(std::min(max_render_points,
                        vins.correct_point_cloud.size()));
                for (size_t i = 0; i < vins.correct_point_cloud.size(); i += stride) {
                    render_point_cloud.push_back(vins.correct_point_cloud[i]);
                }
                for (int i = 0; i < WINDOW_SIZE; ++i) {
                    render_window_Ps[i] = vins.correct_Ps[i];
                    render_window_Rs[i] = vins.correct_Rs[i];
                }
                const Vector3f current_P = render_window_Ps.back();
                if (render_path.empty() || (current_P - render_path.back()).norm() > 0.01f) {
                    render_path.push_back(current_P);
                    render_path_segments.push_back(segmentation_index);
                    if (render_path.size() > 2000) {
                        render_path.erase(render_path.begin(), render_path.begin() + 200);
                        render_path_segments.erase(render_path_segments.begin(),
                                                   render_path_segments.begin() + 200);
                    }
                }
                render_map_ready = true;
            }
        }
            ...
            if (vins.marginalization_flag == vins.MARGIN_OLD &&
                ...
                if (!first_frame && keyframe_freq % LOOP_FREQ == 0) {
                    ...
                    if (vins.Headers[WINDOW_SIZE - 2] == image_buf_loop.front().second) {
                        const std::string pattern_file = configDir + "/brief_pattern.yml";
...
//-(void)loop_thread{
    if (LOOP_CLOSURE && loop_closure == NULL) {
        ...
        TS(load_voc);
        const std::string voc_file = configDir + "/brief_k10l6.bin";
        loop_closure = new LoopClosure(voc_file.c_str(), COL, ROW);
        TE(load_voc);
...
//        LOGI("IMU gyro event timeStamp: %lf", timeStampGyro);
        ...
        // ASENSOR_TYPE_GYROSCOPE is the calibrated sensor. Its values live in
        // the generic vector union member; uncalibrated_gyro is only valid for
        // ASENSOR_TYPE_GYROSCOPE_UNCALIBRATED. The estimator fixes gyro bias
        // to zero, so feeding the calibrated values is required for a
        // consistent IMU model.
        gyro_msg.gyr << gyroEvent.vector.x,
                gyroEvent.vector.y,
                gyroEvent.vector.z;
