...
// After initialization, the trajectory map is the main view and the live
// camera is a small inset. There is no mode switch in the Android UI.
    bool ui_main = false;
    cv::Mat map_preview_cache;
    bool last_frame_is_map = false;
...
// The legacy PnP preview has a separate feature window. Keep the pose source
// unified with VINS so pose, trajectory, and point cloud stay synchronized.
    bool USE_PNP = false;

    // Lock the solved VINS data feedback to featuretracker
    std::mutex m_depth_feedback;

    // Snapshot the back-end map for rendering on the Camera2 callback thread.
    std::mutex m_render_feedback;
    vector<Vector3f> render_point_cloud;
    vector<Vector3f> render_path;
    vector<int> render_path_segments;
    std::array<Vector3f, WINDOW_SIZE> render_window_Ps;
    std::array<Matrix3f, WINDOW_SIZE> render_window_Rs;
    bool render_map_ready = false;
...
public:
    ...
    int getInitFailureCount() {
        return vins.fail_times;
    }

    bool initSucess() {
        return vins.solver_flag == VINS::NON_LINEAR;
    }

    bool lastFrameIsMap() const {
        return last_frame_is_map;
    }
 ...
 */
    ...
    std::string configDir = "/storage/emulated/0/VINS";
    ...
    void processImage(cv::Mat &image, double timeStamp, bool isScreenRotated,
                      bool renderPointCloud = true, cv::Mat *featurePreview = nullptr);
    void drawPointCloudOverlay(cv::Mat &image);
