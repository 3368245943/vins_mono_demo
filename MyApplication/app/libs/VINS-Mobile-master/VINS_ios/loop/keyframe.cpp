...
KeyFrame::KeyFrame(double _header, int _global_index, Eigen::Vector3d _T_w_i, Eigen::Matrix3d _R_w_i,
                   cv::Mat &_image, const std::string &_brief_pattern_file, const int _segment_index)
...
{
    // The packaged OpenCV runtime was built with the legacy GNU STL. Avoid
    // passing libc++ vectors across that ABI boundary; tracked VINS points
    // below still provide the BRIEF descriptors used by loop matching.
    keys.clear();
