...
bool SolveScale(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    printf("initial alignment: scale=%f gravity_norm=%f gravity=(%f,%f,%f)",
           s, g.norm(), g.x(), g.y(), g.z());
    if(fabs(g.norm() - G_NORM) > G_THRESHOLD ||  s < 0)
    {
        printf("initial alignment rejected: gravity_error=%f scale=%f",
               fabs(g.norm() - G_NORM), s);
        return false;
    }
    ...
    printf("initial alignment refined: scale=%f gravity_norm=%f gravity=(%f,%f,%f)",
           s, g.norm(), g.x(), g.y(), g.z());
    if(s > 0.0 )
    {
        printf("initial succ!\n");
