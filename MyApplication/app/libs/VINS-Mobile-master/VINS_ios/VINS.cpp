...
bool VINS::failureDetection() {
    ...
    if (frames_since_reset < 12) {
        ...
        printf("low feature warning %d streak %d\n", f_manager.last_track_num,
               low_feature_failure_streak);
        if (low_feature_failure_streak >= 3) {
...
void VINS::solve_ceres(int buf_num) {
    ...
    const bool is_initial_optimization = solver_flag == INITIAL;
    options.max_num_iterations = is_initial_optimization ? 50 : 10;
    ...
    printf("ceres %s: initial_cost=%f final_cost=%f iterations=%d termination=%d",
           is_initial_optimization ? "initialization" : "tracking",
...
bool VINS::visualInitialAlign() {
    ...
    printf("PS after scale s = %lf: %lf %lf %lf\n", s, Ps[0].x(), Ps[0].y(), Ps[0].z());
